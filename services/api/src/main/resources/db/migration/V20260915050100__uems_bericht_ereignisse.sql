-- =============================================================================
-- UEMS AP-12 IP-4 (Teil 2) — die vier Berichts-Ereignisse im Vokabular
-- =============================================================================
-- Der Bericht-Vertrag (bericht.md §12; events-vocabulary-vectors.json, Block
-- `reserviert`, PR 787) schreibt sie den Berichts-Tabellen zu (V20260915050000):
--
--   bericht_freigegeben           kunde  — Berichtsstand Nr. n freigegeben (F2)
--   bericht_revision_angestossen  cloud  — ein gültiger Stand bekommt einen Anstoß (B7)
--   bericht_entwurf_neu_gebildet  cloud  — Pfad 1 oder 2 hat den Entwurf neu gebildet (EW1)
--   bericht_abgerufen             kunde  — PDF oder CSV eines Stands (DA5)
--
-- Alle vier: Zeitpunkt auf der Messzeit-Achse, Bezug NUR der Bericht (`bericht` =
-- BR-<Jahr>-<Nr.>). Geschrieben werden sie erst von IP-7 (Freigabe), IP-8/IP-9
-- (Anstoß, Entwurf) und IP-10/IP-11 (Abruf); diese Migration legt nur das Wort an.
--
--   1. messreihe_ereignis_vokabular() ist der Stand von V20260915010000 Zeichen für
--      Zeichen — nur vier Zeilen kommen hinten dazu (das 29. bis 32. Wort).
--   2. messreihe_ereignis_bezug_erlaubt() kennt `bericht` als sechsten Schlüssel von
--      `kennungen` (Text, wie die übrigen).
...
-- -----------------------------------------------------------------------------
-- 1. Die vier Berichts-Ereignisse — additiv
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- 2. Der Bezug: `kennungen` darf den Bericht nennen (V20260915010000, geweitet)
-- -----------------------------------------------------------------------------
-- Rahmen wie bisher: jeder Schlüssel ein Text, die Pflicht-Bezüge der Art da, kein
-- Bezug, den die Art nicht kennt. Neu ist nur der sechste Schlüssel `bericht` (die
-- Kennung BR-<Jahr>-<Nr.>).
