...
from voltpilot_optimization.runtime import ServeRuntime, make_health_handler, serve_health
...
def test_the_three_runtime_twins_stay_byte_identical():
    """voltpilot_optimization / _forecast / _market_data ship the SAME file.

    They are three independent distributions with no shared package, so the
    module is duplicated rather than imported (importing would make one service
    a build-time dependency of another). This is the repo's twin discipline -
    change one copy, change all three. Skips outside the monorepo checkout
    (the Docker images only ever contain one package).
    """
    services = pathlib.Path(__file__).resolve().parents[2]
    twins = [
        services / "optimization" / "voltpilot_optimization" / "runtime.py",
        services / "forecast" / "voltpilot_forecast" / "runtime.py",
        services / "market-data" / "voltpilot_market_data" / "runtime.py",
    ]
    missing = [p for p in twins if not p.is_file()]
    if missing:
        pytest.skip(f"sibling package not checked out: {missing[0]}")
    contents = {p: p.read_bytes() for p in twins}
    reference = contents[twins[0]]
    drifted = [str(p) for p, body in contents.items() if body != reference]
    assert not drifted, f"runtime.py twins drifted: {drifted}"
