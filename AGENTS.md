...
- **Der HTTP/JSON-Lesetyp (P5-HTTP): `vp.http.read` neben `vp.mqtt.read`** — derselbe Anschluss ueber eine Web-Auskunft im Heimnetz: Wertepfade mit Platzhalter statt Topic-Filter, kein `stale_s`, ein Flug je Host, und das Geheimnis reist NIE im Flow-Dokument · `der-http-json-lesetyp-p5-http-ebene-1.md`
...
- **Der BMS-Block über den Deye (P4): bei CAN-Kopplung meldet der Wechselrichter die Batterie selbst** — `0x00D2..0x00DF` als OPTIONALER dritter Leseblock; vierzehn Nullen sind keine Batterie, und `0x00D6` ist die ZWEITE `missing`-Ausfahrt für den Ladestand · `der-bms-block-ueber-den-deye-p4.md`
- **Der SCHUTZ-/GRENZBAUSTEIN (P5c): die Batterie sagt, was sie zulässt** — SoC→Strom-Treppe + Zellspannungs-Riegel ergeben `charge/discharge_limit_a` und die Freigaben; sie wirken als BMS-Kappe in `guards.Clamp` und werden NICHT an ein Gerät geschrieben · `der-schutz-grenzbaustein-p5c.md`
