...
const COST_MS = Object.freeze({ modbus_holding: 400, sunspec_model: 400, http_api_key: 250,
  rest_json: 250, rpc_json: 250, ocpp_sampled_value: 0 });

function addressFor(point, discovery) {
  if (!point.address) return null;
  if (point.address.kind === 'modbus_holding') return { start: Math.min(...point.address.registers), count: point.address.width_words };
...
function groupModbus(points, discovery) {
  ...
  for (const selected of points) {
    const key = `${selected.point.family}:${selected.cadence_s}`;
    const list = grouped.get(key) || [];
    const address = addressFor(selected.point, discovery);
    if (address) list.push(Object.assign({ selected }, address));
  ...
  for (const [key, list] of grouped) {
    ...
    for (const item of list) {
      ...
      if (!block || item.start > block.start + block.count + 1 || end - block.start > 120) {
        block = { key, start: item.start, count: item.count, cadence_s: item.selected.cadence_s, points: [] };
        blocks.push(block);
...
function buildPlan(config, options) {
  ...
  for (const s of config.selections || []) {
    ...
    seen.add(s.point_key); const p = resolvePoint(s.point_key, options.discovery);
  ...
  if (!choreography.ok) {
    for (const x of ocpp) rejected.push({ point_key:x.point.point_key,reason:'ocpp_configuration_incompatible' });
  }
  ...
  for (const x of acceptedCandidates.filter((v) => !['modbus_holding','sunspec_model','ocpp_sampled_value'].includes(v.point.source_kind))) {
  ...
  if (budgetReason) {
    return { applied:false, accepted:[], rejected:acceptedCandidates.map((x)=>({point_key:x.point.point_key,reason:budgetReason})).concat(rejected), metrics:{samplesPerMinute:samples,requestsPerMinute:requests,dutyPercent:duty,warning} };
  }
  return { applied:true, revision:config.revision, catalog_version:config.catalog_version,
    accepted:acceptedCandidates.map((x)=>x.point.point_key), rejected, selections:acceptedCandidates,
