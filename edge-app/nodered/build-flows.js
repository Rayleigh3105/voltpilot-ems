...
const controlRouteSource = [
  ...
  "    var powerW = Math.round(Math.abs(dbattKw) * 1000);",
  "    var targetSoc = charging ? 100 : clampPct(socMin);",
  "    var gridChargeAllowed = setpoint.grid_charge_allowed === true;",
  "    var gridChargeBit = (gridChargeAllowed && charging) ? 1 : 0;",
  "    var ratedKw = Number(opts.ratedKw) > 0 ? Number(opts.ratedKw) : 0;",
  "    var exportPct = (pvLimitKw == null || ratedKw <= 0) ? 100 : clampPct((pvLimitKw / ratedKw) * 100);",
  "    var dplanned = [",
  "      { role: 'work_mode', fc: 6, addr: 0x0f01, value: 1, encode: { kind: 'work_mode', mode: 'tou' }, dwell_s: 900, min_change: 0, bench_pending: true },",
  "      { role: 'battery_power', fc: 6, addr: 0x0f3d, value: powerW & 0xffff, encode: { kind: 'watt_u16', kw: dbattKw }, dwell_s: 900, min_change: 50, bench_pending: true },",
  "      { role: 'battery_target_soc', fc: 6, addr: 0x0f44, value: targetSoc, encode: { kind: 'pct', direction: charging ? 'charge' : 'discharge' }, dwell_s: 900, min_change: 1, bench_pending: true },",
  "      { role: 'grid_charge_enable', fc: 6, addr: 0x0f4b, value: gridChargeBit, encode: { kind: 'flag', eeg_gated: true }, dwell_s: 900, min_change: 0, bench_pending: true },",
  "      { role: 'pv_limit', fc: 6, addr: 0x0028, value: exportPct, encode: { kind: 'export_pct', rated_kw: ratedKw, kw: pvLimitKw }, dwell_s: 900, min_change: 1, bench_pending: true }",
  "    ];",
  "    return { adapter: 'solarman_v5', family: family, target: ip + ':' + dport, connection: { ip: ip, port: dport, serial: serial, mb_slave_id: slaveId }, certified: certified, controlEnabled: controlEnabled, writes: [], readbacks: [], planned: dplanned, reason: 'Steuerung f\\u00fcr dieses Modell noch nicht freigegeben' };",
  "  }",
  "  return idle('unbekannte Kommunikationsmethode');",
  "}",
].join('\n');
