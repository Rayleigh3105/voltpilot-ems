...
const sourcesStoreFunc = [
  ...
  "// Feld veroeffentlicht. Drei ausfuehrbare Adapter: modbus_tcp (kompaktes",
  "// Sim-Profil), fronius_sunspec -> sunspec_live (echte SunSpec-Modellerkennung",
  "// ueber Modbus TCP, z. B. eine Fronius-PV als Erzeuger) und goe_http_api (go-e",
  "// Charger ueber ihre lokale HTTP-API, ein Verbraucher). Synchron gehalten mit",
...
const sourcesReadFunc = [
  ...
  "const net = global.get('net');",
  "if (!net) { node.status({ fill: 'red', shape: 'ring', text: 'net fehlt (settings.js)' }); node.error('functionGlobalContext.net in settings.js setzen', msg); return null; }",
  "// http/https are needed only for the go-e (goe_http_api) branch; the modbus /",
  "// sunspec branches do not use them, so a missing global only fails that branch",
  "// (with a named warn), never the whole poll.",
  "const http = global.get('http');",
  "const https = global.get('https');",
...
const sourcesNodes = [
  {
    ...
    info: [
      ...
      'Erzeuger) und goe_http_api-Quellen (go-e Charger ueber ihre lokale HTTP-API,',
      'ein Verbraucher) werden gelesen; eine solarman_v5-Quelle wird erkannt, ihr',
