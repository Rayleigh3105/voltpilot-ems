...
const controlRouteSource = [
  ...
  "    var slot = 0;",
  "    var dplanned = [",
  "      { role: 'work_mode', fc: writeFc, addr: reg.workMode, value: 0, encode: { kind: 'work_mode', enum: 'export_first' }, dwell_s: 900, min_change: 0, bench_pending: true },",
  "      { role: 'tou_enable', fc: writeFc, addr: reg.touEnable, value: 0x00ff, encode: { kind: 'tou_mask', all_week: true }, dwell_s: 900, min_change: 0, bench_pending: true },",
  "      { role: 'program_time', fc: writeFc, addr: reg.progTimeBase + slot, value: 0, encode: { kind: 'program_time_hhmm', hhmm: 0 }, dwell_s: 900, min_change: 0, bench_pending: true },",
  "      { role: 'battery_power', fc: writeFc, addr: reg.progPowerBase + slot, value: powerReg, encode: { kind: 'watt_scaled_u16', scale: powerScale, kw: dbattKw }, dwell_s: 900, min_change: 50, bench_pending: true },",
  "      { role: 'battery_target_soc', fc: writeFc, addr: reg.progSocBase + slot, value: targetSoc, encode: { kind: 'pct', direction: charging ? 'charge' : 'discharge' }, dwell_s: 900, min_change: 1, bench_pending: true },",
  "      { role: 'grid_charge_enable', fc: writeFc, addr: reg.progChargeBase + slot, value: chargeEnum, encode: { kind: 'charge_enum', eeg_gated: true, disabled: 0, grid: 1 }, dwell_s: 900, min_change: 0, bench_pending: true }",
  "    ];",
  "    if (pvLimitKw != null) { var capW = Math.max(0, pvLimitKw * 1000); dplanned.push({ role: 'pv_limit', fc: writeFc, addr: reg.exportLimit, value: Math.round(capW / reg.exportLimitScale) & 0xffff, encode: { kind: 'feed_in_cap_w', scale: reg.exportLimitScale, kw: pvLimitKw }, dwell_s: 900, min_change: 1, bench_pending: true }); }",
...
const controlReleaseSource = [
  ...
  "  var comm = selection.communication;",
  "  if (tier === 3 && comm === 'solarman_v5') {",
  "    var DEYE_REG = { hybrid_3p: { touEnable: 0x0092 }, hybrid_1p: { touEnable: 0x00f8 } };",
  "    var dport = Number(conn.port) > 0 ? Number(conn.port) : 8899;",
  "    var serial = conn.serial;",
  "    var slaveId = Number(conn.mb_slave_id) > 0 ? Number(conn.mb_slave_id) : 1;",
  "    var reg = Object.prototype.hasOwnProperty.call(DEYE_REG, family) ? DEYE_REG[family] : null;",
  "    var writeFc = (conn.control_write_fc === 6 || conn.control_write_fc === '6') ? 6 : 16; // FC16 default, FC6 flip-back (mirrors controlRoute)",
  "    var dplanned = reg ? [{ role: 'tou_enable', fc: writeFc, addr: reg.touEnable, value: 0, encode: { kind: 'tou_mask', all_week: false, release: true }, dwell_s: calibration ? 0 : 900, min_change: 0, bench_pending: true }] : [];",
  "    var drb = reg ? [{ role: 'tou_enable', fc: 3, addr: reg.touEnable, expect: 0, tolerance: 0 }] : [];",
...
const controlPlanFunc = (selectionExpr) => [
  ...
  "  var rel = controlRelease(sel, { calibration: sp.calibration === true });",
