...
const controlReleaseSource = [
  ...
  "    var slaveId = Number(conn.mb_slave_id) > 0 ? Number(conn.mb_slave_id) : 1;",
  "    var reg = Object.prototype.hasOwnProperty.call(DEYE_REG, family) ? DEYE_REG[family] : null;",
  "    var dplanned = reg ? [{ role: 'tou_enable', fc: 6, addr: reg.touEnable, value: 0, encode: { kind: 'tou_mask', all_week: false, release: true }, dwell_s: 900, min_change: 0, bench_pending: true }] : [];",
  "    var drb = reg ? [{ role: 'tou_enable', fc: 3, addr: reg.touEnable, expect: 0, tolerance: 0 }] : [];",
  "    return { adapter: 'solarman_v5', family: family, tier: tier, certified: certified, mode: 'release', target: ip + ':' + dport, connection: { ip: ip, port: dport, serial: serial, mb_slave_id: slaveId }, writes: certified ? dplanned : [], readbacks: certified ? drb : [], planned: dplanned, reason: certified ? undefined : 'Steuerung f\\u00fcr dieses Modell noch nicht freigegeben' };",
  "  }",
  "  if (tier === 2) {",
  "    var v2port = Number(conn.port) > 0 ? Number(conn.port) : 502;",
  ...
  "    ];",
  "    return { adapter: 'modbus_tcp', family: family, tier: tier, certified: certified, mode: 'release', profile: family, target: ip + ':' + port, connection: { ip: ip, port: port, unit_id: unitId }, writes: certified ? planned : [], readbacks: certified ? readbacks : [], planned: planned, reason: certified ? undefined : 'Modell noch nicht freigegeben' };",
  "  }",
  "  if (tier === 1 && comm === 'fronius_solar_api') {",
  "    var fport = Number(conn.control_port) > 0 ? Number(conn.control_port) : 502;",
  "    var funitId = Number(conn.control_unit_id) > 0 ? Number(conn.control_unit_id) : 1;",
