...
const sourcesStoreFunc = [
  ...
  "let deferred = 0;",
  "list.forEach((s) => {",
  "  if (!s || (s.role !== 'pv-generation' && s.role !== 'grid-meter')) return;",
...
const sourcesReadFunc = [
  ...
  "if (!plans.length) { node.status({ fill: 'grey', shape: 'ring', text: 'keine Quellen' }); return null; }",
  ...
  "  const key = 'src_warn_' + plan.id;",
  "  const last = context.get(key) || 0;",
  "  if (Date.now() - last < 60000) return;",
  "  context.set(key, Date.now());",
  "  node.warn('Quelle ' + plan.id + ' (' + plan.adapter + ' ' + plan.conn.ip + ':' + (plan.conn.port || 502) + '): ' + why);",
  "};",
  "const s16 = (v) => { v &= 0xffff; return v > 0x7fff ? v - 0x10000 : v; };",
  "const r3 = (x) => Math.round(x * 1000) / 1000;",
  "const frameLen = (buf) => (buf.length < 6 ? null : 6 + buf.readUInt16BE(4));",
  ...
  "        const out = await sunspecRead({ ip: plan.conn.ip, port: plan.conn.port, unitId: plan.conn.unit_id, invertGridSign: !!plan.conn.invert_grid_sign, modelType: plan.conn.model_type }).catch((e) => { why = 'SunSpec-Lesefehler: ' + (e && e.message ? e.message : e); return null; });",
