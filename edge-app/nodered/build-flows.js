...
const controlRouteSource = [
  ...
  "    // N1 fix: use the EXPLICIT power_scale (1 or 10); confirmed=false on fallback.",
  "    var psv = Number(conn.power_scale); var powerScale = (psv === 1 || psv === 10) ? psv : 1; var powerScaleConfirmed = (psv === 1 || psv === 10);",
  ...
  "      var out = { adapter: 'solarman_v5', family: family, target: ip + ':' + dport, connection: { ip: ip, port: dport, serial: serial, mb_slave_id: slaveId }, certified: certified, controlEnabled: controlEnabled, calibration: calibration === true, writes: dWriteAllowed ? execWrites : [], readbacks: dWriteAllowed ? readbacks : [], planned: planned };",
...
const controlReleaseSource = [
  ...
  "    return { adapter: 'solarman_v5', family: family, tier: tier, certified: certified, calibration: calibration, mode: 'release', target: ip + ':' + dport, connection: { ip: ip, port: dport, serial: serial, mb_slave_id: slaveId }, writes: releaseAllowed ? dplanned : [], readbacks: releaseAllowed ? drb : [], planned: dplanned, reason: releaseAllowed ? undefined : 'Steuerung f\\u00fcr dieses Modell noch nicht freigegeben' };",
  "  }",
  "  if (tier === 2) {",
  "    var v2port = Number(conn.port) > 0 ? Number(conn.port) : 502;",
...
const controlPlanFunc = (selectionExpr) => [
  ...
  "var plan = controlRoute(sel, sp, { snapshot: snapRegs });",
  ...
  "  var rel = controlRelease(sel, { calibration: sp.calibration === true, snapshot: snapRegs });",
