...
- **Edge-Updates laufen über das Portal, das Gerät wendet selbst an:** der Sidecar `vp-edge-updater` ist seit dem 26.08.2026 ein **gewöhnlicher Dienst** - kein Compose-Profil, kein Geräte-Schalter, kein Handgriff vor Ort. `./update.sh` bringt ihn auf einer Bestandsbox EINMAL mit; danach genügt im Portal *Plattform → Edge-Updates*: Release wählen, Geräte ankreuzen, **Aktualisieren**. Was ein Update noch aufhalten kann (Signaturkette, Anti-Rollback-Boden, Datenstand, Platte), das Aufräumen alter Abbilder und die Fehlerinjektions-Matrix: **[`docs/ota-autonomie.md`](../docs/ota-autonomie.md)**.
