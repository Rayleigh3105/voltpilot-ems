...
- **Autonome Updates (OTA Stufe 3) - GEBAUT, aber nirgends eingeschaltet:** ein zusätzlicher Sidecar `vp-edge-updater` kann den Tausch selbst fahren (holen, sequenziert tauschen, Selbsttest, bei Bedarf zurücknehmen). Er hat **zwei unabhängige Tore, und beide sind zu**: das Compose-Profil `ota` (ohne `--profile ota` läuft der Container gar nicht) und der Schalter je Gerät (`/data/ota/autonomy.json`, Vorgabe AUS). Eine Box ohne beides verhält sich zeichengleich wie hier beschrieben. `./update.sh` erkennt einen laufenden Sidecar selbst und nimmt ihn mit (`--ota` erzwingt es bei gestoppten Containern) - **ohne diese Erkennung würde `up -d --remove-orphans` ihn als Waise entfernen.** Einschalten, Prüfstand-Voraussetzung (die Inverter-Neutral-Zeit T), Registry-Token je Gerät und die Fehlerinjektions-Matrix: **[`docs/ota-autonomie.md`](../docs/ota-autonomie.md)**.
