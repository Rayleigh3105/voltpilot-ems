...
- **Config** (`core/internal/config`): env > `config.json` (in the data dir or `VP_CONFIG`) > defaults. Defaults: portal `https://portal.voltpilot.de`, broker `mqtt.voltpilot.de:8883`, data dir `/data`, web `:8484`, bus `:1883`. An unset reference is generated once and persisted (`/data/ref`).
...
- **Cloud link** (`core/internal/cloud`): ONE outbound mTLS MQTT connection (port 8883, cert CN = device id, username/clientid derived by the broker). Publishes the **frozen** telemetry contract + status heartbeat, subscribes the retained schedule topic. Additively, it bridges the v2 `measurement-config` / `measurement-config-status` / `measurement-samples` contracts on the already identity-scoped `v2/#` ACL grant; old edge versions simply never subscribe to the new desired-state topic.
...
- **Additional measurements** (`core/internal/measurements` + `nodered/measurements`): the retained cloud desired state is identity-checked, monotone and atomically persisted before it is exposed on `edge/measurements/config`. Layer 1 atomically activates a budgeted read-only poll plan and returns a retained accepted/rejected status. Sample batches use their own bounded durable QoS1 outbox with monotone sequence, ordered reconnect replay and explicit `gap`/`dropped_samples`; they never alter or derive from the frozen `edge/telemetry` payload.
