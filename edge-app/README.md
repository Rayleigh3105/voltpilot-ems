...
- **Web app + health** (`core/internal/web`): German, read-only status + the inverter-selection form, self-contained (design-system token values baked in). `GET /health` for watchdogs; structured JSON logs on stdout; `restart: unless-stopped` in compose.
