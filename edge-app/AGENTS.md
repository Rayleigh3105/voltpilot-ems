...
- **Die SPEISER-BINDUNG auf der Box (P6)** — kein neuer Weg: die Bindung reist als `role_assignment`; `user-defined-battery` bekommt keine Vorgabe-Rolle mehr, und der Speicher-Knoten trägt `soc_source` + `limits` · `die-speiser-bindung-auf-der-box-p6.md`
- **vp-soc-derive (P5b Ebene 2): die SoC-Ableitung auf der Box** — Palette 0.11.0, haengt an einer KANTE hinter dem Lese-Knoten (nie am Takt); kein Ladestand ohne Eingang, ein eingefrorener Wert bekommt nie eine frische Zeit · `vp-soc-derive-p5b-die-soc-ableitung.md`
- **vp-limit-guard (P5c): die Schutzgrenzen auf der Box** — Palette 0.13.0, am ENDE der Kette; die Grenzen werden BEREITGESTELLT, nie geschrieben, und binden als `guards.Limits.Bms` beide Klemm-Pfade (v1 wie v2) · `vp-limit-guard-p5c-die-schutzgrenzen.md`
- **vp-http-read (P5-HTTP): die Web-Auskunft des BMS auf der Box** — Palette 0.12.0, ein GET je Takt, Wertepfade mit `*` statt Topic-Filter, EIN Flug je Host; das Geheimnis kommt aus `edge/entities/{id}/config`, nie aus dem Flow · `vp-http-read-p5-http-die-web-auskunft.md`
