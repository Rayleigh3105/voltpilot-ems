...
-- + V20260702000000__forecast_model_column.sql so `docker compose up` gives the
-- forecast service its table without a Flyway run. Keep this DDL in sync with
-- those migrations (the migrations are the source of truth in staging/prod;
-- the api's V20260701040000 also retrofits the model column idempotently).
-- Forecasts are timeseries (architecture section 10). Every row is tagged with
-- the producing MODEL id (shadow-mode forecasting - docs/forecasting.md).
-- =============================================================================
