...
| Wortfolge | `big` = höherwertiges Wort zuerst, `little` = niederwertiges zuerst; gilt für jeden 32-bit-Wert (Prüfwert, Kennung, Messwerte). Im Messpunkt-Katalog: `big` → `big`, `little` → `word_little_byte_big` (`catalog/measurement-points/schema/catalog.schema.json:162`) | zu erheben: Hardwareblatt §3, Pilotschritt 2 |
