...
| `mqtt-measurement-config.valid.per-component.json` | mqtt-measurement-config | valid — additive `entity_id` binds one selection to one component (Stufe 3b); the second entry keeps the device-wide box semantics |
