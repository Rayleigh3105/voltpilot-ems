...
| [`wago-simulator-vectors.json`](./wago-simulator-vectors.json) | AP-05 IP-12: Simulator-Vorstufe zum Registerbild — fünf Fälle (Normallast, Rücksetzung, Herzschlag steht, Version fremd, Karte fehlt) auf dem Register-Store `edge-app/nodered/vp-palette/test/fixtures/wago-registerbild-store.js`. Aufbau und Vokabular kommen aus `wago-registerbild-vectors.json`; jeder Fall `herkunft: simulator`, `belegt: false` — der Simulator dient dem Bauen, nie dem Beleg. |
