...
| [`measurement-budget-vectors.json`](./measurement-budget-vectors.json) | AP-07 E10/A16: gemeinsame Kosten, Blockgrößen und Grenzfälle für Java/Node-RED; [Lesewege und Mischbetrieb](../../agents/root/uems-messbudget-vertrag.md). |
...
| [`steuerungsverbund.md`](./steuerungsverbund.md) + [`verbund-anteil-vectors.json`](./verbund-anteil-vectors.json) | AP-15 IP-2 (NW-1): Anteile je Richtung, Übergangsstand des Zweischritts, Dokument-Prüfung der Box und die geschlossenen Vokabulare der Gemeinsamen Steuerung (Java `SteuerungsverbundAnteile` ⟷ Python-Referenz; Go mit IP-17). |
| [`steuerungsverbund-objekt-vectors.json`](./steuerungsverbund-objekt-vectors.json) | AP-15 IP-4: Regeln des Verbund-Objekts — T1 Heimat der Box, T2 genau ein Netzanschluss (W3), genau eine führt, Messpunkt = Datenquelle der Anlage und vom Mitglied gelesen, T6 Lesen macht kein Mitglied (Java `SteuerungsverbundRegeln`; Tabellen V20260921140000). |
...
| [`bezugsbasis.md`](./bezugsbasis.md) + [`bezugsbasis-vectors.json`](./bezugsbasis-vectors.json) + [`bezugsbasis.schema.json`](./bezugsbasis.schema.json) | Regeln und gemeinsame Testvektoren der Bezugsbasis einer Kennzahl (AP-17): Referenzperiode, Basiswert Σ ÷ Σ, Modelle und Gradtage, Grenzen, Urteil, Zeitraum. |
| [`verbesserung.md`](./verbesserung.md) + [`verbesserung-vectors.json`](./verbesserung-vectors.json) + [`verbesserung.schema.json`](./verbesserung.schema.json) | Regeln und gemeinsame Testvektoren für Energieziele, Maßnahmen und Abweichungen (AP-18): Wirkung Σ ÷ Σ über die Nachher-Monate, Ziel-Stand mit Vorschlag, Frist, Kundensätze. Aufrufer: die Routen `/api/v1/energieziele`, `/api/v1/massnahmen`, `/api/v1/abweichungen`; Auffälligkeit und Anstoß am Vorgang haben keinen eigenen Vertrag, sie sind Vermerke der Naht (`docs/agents/root/uems-verbesserung-naht.md`). |
| [`energiemanagement.md`](./energiemanagement.md) + [`energiemanagement-vectors.json`](./energiemanagement-vectors.json) + [`energiemanagement.schema.json`](./energiemanagement.schema.json) | Regeln und gemeinsame Testvektoren für das Energiemanagement (AP-19): Überprüfung, Audit- und Bewertungs-Rhythmus, Frist der Feststellung, Wiedervorlage, Vergleich Anwendungsbereich ⟷ Betrachtungsumfang, Verzeichnis-Zeile, kanonische Zahlform der Prüfsumme, Kundensätze. Aufrufer: die Routen unter `/api/v1/energiemanagement` (Dokumente, Personen und Aufgaben, Verzeichnis, Nachweise, Audits, Feststellungen, Wiedervorlage); die Managementbewertung ist die Vorlage Nr. 7 von `bericht-vorlagen.json`; keine Ereignisse. Einstieg: `docs/agents/root/uems-energiemanagement-abschluss.md`. |
| [`kennzahl-variablen-vorschlag.md`](./kennzahl-variablen-vorschlag.md) + [`kennzahl-variablen-vorschlag.schema.json`](./kennzahl-variablen-vorschlag.schema.json) + [`variablen-vorschlag-vectors.json`](./variablen-vorschlag-vectors.json) | AP-17 IP-11a: der Variablen-Vorschlag einer Kennzahl aus dem Energieeinsatz (V4) — Verweise als Kandidaten, Wortlaute „ohne Zahl“, Abhängigkeits-Regel G4 (Java ⟷ TS); nur Lesen. |
...
| [`wago-simulator-vectors.json`](./wago-simulator-vectors.json) | AP-05 IP-12: Simulator-Vorstufe zum Registerbild — sechs Fälle (Normallast, Rücksetzung, Herzschlag steht, Version fremd, Karte fehlt, Bereichsbegrenzung — S6 kam mit IP-8) auf dem Register-Store `edge-app/nodered/vp-palette/test/fixtures/wago-registerbild-store.js`. Aufbau und Vokabular kommen aus `wago-registerbild-vectors.json`; jeder Fall `herkunft: simulator`, `belegt: false` — der Simulator dient dem Bauen, nie dem Beleg. |
