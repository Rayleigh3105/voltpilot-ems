...
| `POST …/fassungen/{n}/freigeben` | F1: ohne Vier-Augen gibt die Person den **Entwurf** mit Begründung frei (`freigabe_*`); F2: mit Vier-Augen bestätigt eine **zweite Person** den Antrag (`entscheidung_*`; nicht, wer die Fassung gebildet oder beantragt hat; Rolle KA/EM). `freigegeben_am` = jetzt (Beginn der Wiedervorlage, F5). F4: die laufende freigegebene Vorgängerin endet am Vortag des `gilt_ab` (`gilt_bis`, `beendet_grund` „abgelöst durch Fassung n“, Protokoll `fassung_beendet`), sonst bleibt sie byte-gleich. Protokoll `fassung_freigegeben`; freigegeben und abgelehnt kehren nie zurück (Trigger). **Nachlese 2 (F3, E5 = A):** vor dem Zustandswechsel wird die Grundlage aus denselben Eingaben neu gebildet (Referenzperiode, Methode, Kennzahl-Fassung am letzten Tag, Nenner, zweite Variable — nach G4 die abgelehnte) und ihre Prüfsumme mit der gespeicherten verglichen; weicht sie ab (neue Version eines zitierten Werts, andere Bezugsgrößen- oder Kennzahl-Fassung), ist der Entwurf veraltet: 409 `entwurf_veraltet` „Die Grundlage hat sich seit dem Entwurf geändert – bilden Sie den Entwurf neu.“ mit `fassung` und `grund` (`grundlage` · `faktoren`), nichts wird entschieden; der Kunde bildet neu (`POST …/fassungen` ersetzt den offenen Entwurf). Ein **Antrag** ist eingefroren: dort heißt der Satz „… seit dem Antrag geändert – lehnen Sie den Antrag ab und bilden Sie den Entwurf neu.“, auch wenn sich nur ein Faktor geändert hat. Die Faktoren am Freigabetag regelt §17 | 409 `vieraugen_beantragen` (Entwurf bei Vier-Augen) · `fassung_freigegeben` · `fassung_abgelehnt` · `gilt_ab_vor_vorgaengerin` · `bezugsbasis_beendet` · `entwurf_veraltet`; 422 `begruendung_fehlt` · `vieraugen_urheber` · `faktor_ungueltig` (§17); 403 `recht_fehlt` · `vieraugen_rolle` |
...
**Freigabetag** (Nachlese 2): `freigeben` — mit Vier-Augen schon `beantragen` — bewertet die gespeicherten Faktoren am
Freigabetag (heute in der Zeitzone der Kennzahl) gegen den Vorschlag (§14) neu. Hat sich Kennung, Bezeichnung, Wert,
Einheit oder Gültigkeit eines Faktors geändert, wird **jede** Kopie mit `kopie_am` = Freigabetag neu gezogen: Zeilen in
`bezugsbasis_faktor` neu (die alten aufgehoben), Block `faktoren` und Prüfsumme neu — in derselben Transaktion VOR dem
Zustandswechsel, weil der Trigger ab dem Antrag einfriert; der Basiswert bleibt. Das Protokoll trägt es am Wort des
Entscheids (`fassung_freigegeben` bzw. `fassung_beantragt`, kein neues Vokabular-Wort): `alt` = `{pruefsumme, faktoren[]}`
(Sätze), `neu` zusätzlich `anlass: faktoren_neu_kopiert`, `kopie_am`, `faktoren[]`. Ohne Änderung bleibt alles
byte-gleich (Stichtag des Entwurfs). Nennt der Vorschlag ein verwiesenes Objekt am Freigabetag nicht mehr → 422
`faktor_ungueltig` „Den statischen Faktor Fläche G-2 nennt die Struktur der Geltung am 05.01.2027 nicht mehr – bilden
Sie den Entwurf ohne ihn neu.“ mit `art`, `objekt_id`, `stichtag`; der Entwurf bleibt. Nach einem Antrag ist eine
geänderte Kopie nicht mehr möglich → 409 `entwurf_veraltet` (`grund: faktoren`, §13). Die **Kopie** steht in
...
Anbau der Halle 2 (3 100 → 3 400 m² ab 01.01.2027) die Fassung im Struktur-Läufer an (`struktur_geaendert`). Wird derselbe
Entwurf erst am 05.01.2027 freigegeben, kopiert die Freigabe 3 400 m² gültig ab 01.01.2027 (Nachlese 2). Geprüft in
