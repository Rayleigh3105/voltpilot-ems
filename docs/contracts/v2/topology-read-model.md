...
- **Konflikt = Ablösung (is_primary).** Höchstens ein maßgeblicher Wert je (Gerät, Rolle) — **DB-garantiert** über den partiellen Unique-Index `uq_entity_role_primary (entity_id, role) WHERE is_primary` (`V20260914100200`). Ein zweiter Wert auf dieselbe Rolle ersetzt den vorigen (demote-then-insert in einer Transaktion); die Antwort nennt den abgelösten Wert. Ein **nebenläufiger** zweiter Schreiber läuft sauber in den Constraint → **409** (statt einer stillen Doppelzählung).
- **Same-Site.** Ein zugeordneter Gesamtwert muss zur selben Anlage gehören wie das Gerät — die Ableitung Messstelle→Gerät→Anlage läuft über die Terme des Gesamtwerts (transitiv über verkettete Messstellen); ein fremd-anlagiger Gesamtwert wird abgelehnt (**400**), sonst summierte sich ein fremder Wert unter dem Gerätenamen dieser Anlage.
