...
| **G3 / E2 = A** | Jeder Anteil ≥ Geräte-Rückfall der Box und ≤ ihrer Nennleistung. Passt die Summe der Rückfälle nicht in `verteilbar`: `auslegung_passt_nicht` — in **beiden** Richtungen eine Ablehnung, keine Warnung. Eine Richtung ohne Grenze fehlt und passt nicht — außer der Einspeisung, wenn das Grenzblatt ausdrücklich „keine Einspeisegrenze“ trägt (AP-15 Folge, Captain 23.09.2026): dann ist sie **unbegrenzt**, ohne Anteil und ohne Wächter, und nur der Bezug wird aufgeteilt (`SteuerungsverbundAbleitung.passt(auslegung, unbegrenzt)`). | `anteile` |
...
| **Übergangszuschlag** (Captain 22.09.2026, „Puffer einrechnen“, Lesart B) | Fällt die führende Box aus, halten ihre Speicher bis zum Geräte-Rückfall den letzten Sollwert (Simulator-Befund IP-29 A2/A15: Viertelstunde 100,171/100,258 kW bei 100 kW). Puffer `Z` = Rückfallzeit / 900 s × Leistung der Speicher an der führenden Box — Einspeisung `asset.max_discharge_kw`, Bezug `asset.max_charge_kw` (Summe aller `battery`-Zeilen dieser Box oder ohne Box), Rückfallzeit = größtes `nach_s` ihrer Speicher-Komponenten in dieser Richtung (`komponente_geraete_rueckfall`, sonst Katalog), ohne Angabe 60 s — auf 0,1 kW **auf**gerundet. `Z` geht vom Rest über den Rückfällen ab, bevor G4 verteilt, höchstens so viel, wie dort ist; der Rest fehlt (`zuschlag_fehlt_kw`) und wird mit dem Handgriff am Gerät genannt (Rückfallwert senken). **Nie eine Ablehnung**: Urteil und `verteilbar = Grenze − Vorbehalt` bleiben wie ohne ihn; die Box rechnet nichts davon. Ahrenberg A2: Einspeisung 40/60 „passt“, davon fehlen 4 kW (Rückfallwert K-1 auf höchstens 36 kW); Bezug 0/73 statt 0/77. Ohne Speicher an der führenden Box `Z` = 0 — byte-gleich wie vorher. | `uebergangszuschlag` |
...
| **T4, G5, Y1 Dokument-Prüfung** | Die Box prüft jedes Anteils-Dokument in dieser Reihenfolge: Mandant und Anlage = ihre Identität (`fremde_anlage`) · die eigene Kennung steht in BEIDEN Richtungen der Tabelle — unbekannt ist keine Null (`box_fehlt_im_dokument`); fehlt die Einspeiseseite ganz (weder `verteilbar` noch Tabelle: Einspeisung unbegrenzt, AP-15 Folge, [Anteils-Dokument](mqtt-verbund-anteile.md) §2), nur im Bezug · Epoche und Revision steigen nur: kleinere Epoche oder gleiche Epoche mit kleinerer Revision (`revision_aelter`) · je Richtung Summe ≤ `verteilbar` des Dokuments, exakt ohne Rundung (`summe_ueber_verteilbar`). Dieselbe Revision noch einmal (gespeichertes Dokument nach Wiederverbindung) wird angenommen. Eine neue Epoche setzt nur das Scharfschalten. | `dokument_pruefen` |
...
| `uebergangszuschlag` | `…uebergangszuschlag` + `anteile(…, zuschlag, …)` | `…uebergangszuschlag` + `anteile(…, zuschlag_kw=)` | — (die Box rechnet keine Auslegung; `_abweichungen`) |
...
| **A14/R17 Box-Tausch** (Folge zu IP-30) | `vorgaenger_mitglied_id` + `anteil_kennung` am Mitglied, Protokoll `art = 'box_getauscht'` (`V20260922150000`) | kein Urteil: `uems/GemeinsameSteuerungBoxTausch` in der Transaktion von „Box tauschen“ (AP-06 E7) — die Mitgliedschaft endet zum Tauschzeitpunkt, die Nachfolgerin beginnt dort mit derselben Rolle, demselben Messpunkt, demselben Signal und den Geräte-Angaben (aufgehoben/neu), `bestaetigt_am` NULL = „wartet auf Bestätigung“ (I4); sie bekommt das jüngste gespeicherte Dokument mit DERSELBEN Epoche/Revision, der Eintrag der Vorgängerin auf sie umgeschlüsselt (kein neues Dokument, kein Zielstand, keine andere Box bekommt etwas); jeder Leser des Dokuments (Zielstand, Rückspielen, Kunden-GET, Planer `_UMSCHLUESSELN`) schlüsselt über `anteil_kennung` um; bis zur Bestätigung rechnet der Planer sie belegt; die Sprungprobe gilt über die Tausch-Linie weiter (die Quellen reisen 1:1 mit), Entwerten trifft die ganze Linie. Ohne Gemeinsame Steuerung eine Abfrage, keine Zeile |
| **§5.5 Ausscheiden** (Folgepaket, I3/I4, V5, R12) | `scheidet_aus_seit` · `scheidet_aus_wartet_auf` (`quittung` · `betreiber`) · `vom_netz_bestaetigt_am` am Mitglied, Dokument-`anlass = 'ausscheiden'`, Protokoll `art = 'mitglied_ausgeschieden'` (`V20260922160000`) | kein Urteil: `uems/GemeinsameSteuerungAusscheiden` + `SteuerungsverbundAnteilDienst#ausscheidenBeginnen/#ausscheidenPruefen`. Übergang: die ausscheidende Box je Richtung auf den Rückfall ihrer Geräte (G3; 0 ohne Geräte oder nach „vom Netz“), die anderen je Box das Kleinere (G5); das Dokument trägt das Ziel der VERBLEIBENDEN Boxen nach G4 mit dem Rückfall der ausscheidenden im Vorbehalt. Ziel erst, wenn die ausscheidende Box das Dokument quittiert hat (`quittung`) oder der Betreiber bestätigt, dass ihre Geräte vom Netz sind (`betreiber` nach dem Abmelden wartet NUR darauf); dann endet die Mitgliedschaft (zeitgültig), der Rückfall ohne Bestätigung geht in den Vorbehalt (Protokoll `vorbehalt`, Grund `mitglied_ausgeschieden`), die verbleibenden Boxen bekommen den Rest — ohne Quittung steht der Übergang (R12). Die Box bekommt kein Dokument mehr und hält ihr letztes (V5); `altAusDokumenten` zählt nur wirksame Mitglieder. Nie scharf: sofort beendet (Grund `ohne_anteile`) |
...
| `POST /api/v1/admin/…/mitglieder/{boxId}/ausscheiden-bestaetigen` | `plattform.betrieb` | „Geräte sind vom Netz“ (I4): `vom_netz_bestaetigt_am`; der Rückfall bleibt nicht reserviert, das Ziel geht ohne Quittung; 409 `scheidet_nicht_aus` · `bereits_bestaetigt` |
| `POST /api/v1/admin/…/steckerprobe` | `plattform.betrieb` (nur Plattform-Rolle, I4/NW-8) | `{von,bis,box_id,bemerkung}` benennt den Kabelzug; höchstens 31 Tage, ein Eintrag je Probe; der GET rechnet den Grenz-Nachweis für genau `[von,bis)` |
