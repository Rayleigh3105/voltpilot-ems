...
| Viertelstunden `messreihe_viertelstunde` | 30-Tage-Chunks, 3 653 Tage, RLS + FORCE; dauerhafte Arbeitsliste; **keine Kompression** (RLS schliesst sie aus — darum kann ein `ADD COLUMN` hier auch nie auf einen komprimierten Chunk treffen); seit `V20260918104000` zwei nullbare Anteils-Spalten, **0 B je Zeile solange NULL** (Null-Bitmap 6 → 7 B, Zeilenkopf bleibt `MAXALIGN(23+7) = 32 B`) | `services/api/src/main/resources/db/migration/V20260912170000__uems_messreihe_viertelstunde.sql:239-258,293-314,364-374` |
