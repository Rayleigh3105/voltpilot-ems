...
| Steuerung „Diese Anlage misst nur.": `nurMessenLeerzustand` (Fassung Standort ohne/mit Funktion, A8 ohne steuerbare Komponente), `NurMessenHinweis` ÜBER den Zonen | `src/steuerungArea.ts`, `components/NurMessenHinweis.tsx`, `pages/SteuerungSection.tsx` · `steuerungLeerzustand.test.ts`, `pages/SteuerungSection.test.tsx` |
| Karte „Funktionen" (unter der Anlagen-Tabelle beider Ebenen) und Leerzustand der Standort-Übersicht: `funktionenKarte`, `standortLeerzustand` | `src/uebersicht.ts`, `components/FunktionenKarte.tsx`, `components/PortfolioCockpit.tsx` · `funktionenKarte.test.ts`, `components/Uebersicht.test.tsx` |
...
1. **Der nächste Schritt ist ein benannter Hinweis, kein Knopf** (Captain zu PR 771): die Assistenten
   IP-9a/IP-10a gibt es noch nicht, ein Knopf ohne Ziel wäre eine Sackgasse. Einen Knopf gibt es nur mit
   heutigem Ziel — allein „Gerät anbinden" (A8, Anlage › Komponenten). Kommt ein Assistent, wird sein Hinweis
...
6. **Der Leerzustand der Steuerung steht ÜBER den Zonen, nie statt ihrer.** Steuerart, „＋ Neue Regel",
   „Komponente anlegen" und Schutz bleiben die heutigen Wege (die Variante „statt der Zonen" nahm 6 Knöpfe und
   5 Abschnitte weg). Er erscheint nur bei Teilnahme `kein_objekt`; jede andere Teilnahme lässt die Seite
   unverändert. Steuerbar = Speicher (`battery-hybrid`) + Einträge der Verbraucher-Zone — ohne Zone
   unbekannt, nie „keine".
...
```bash
(cd frontend/portal && npx vitest run src/anlageGeld.test.ts src/steuerungLeerzustand.test.ts src/funktionenKarte.test.ts \
