...
| `frontend/portal/src/copy.test.ts` | **new** | The copy guard: reads the customer-facing source files (`node:fs`, the `migration.test.ts` teardown-guard precedent) and fails on forbidden vocabulary — `Entität`, `Messpunkt`, `Quelle` (as an entity noun), `Mess-Einheit`, `Modul`, `MILP`, `Optimizer`, `Broker`, `Flow-Dokument`, `Anlagenteil`, and the dropped request-wall wording `Angefragt` / `in Vorbereitung` / `VoltPilot richtet ein` (M3: every profile is a direct customer toggle) — outside `src/pages/admin/**` and the technical panels. |
