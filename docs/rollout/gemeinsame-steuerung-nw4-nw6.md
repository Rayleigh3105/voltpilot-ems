...
| A14/R17 Box-Tausch | `T/uems/GemeinsameSteuerungBoxTauschApiTest` (R17 mitsteuernd, führende Box, ohne Verbund), `T/uems/GemeinsameSteuerungApiTest#mitgliedBestaetigenNurDiePlattformUndNurEinmal`, `O/test_plan_je_box.py::test_belegte_box_bekommt_kein_dokument…[unbestaetigt]` | geschlossen (Folgepaket zu IP-30): Tausch überträgt Mitgliedschaft, Geräte-Angaben und Dokument (gleiche Revision), Nachfolgerin wartet auf Bestätigung; offen: eine führende Nachfolgerin bekommt vor der Bestätigung weiter den Plan der führenden Box |
