...
describe('customer visibility (audit N-1)', () => {
  it('flags exactly the not-offered nodes: undeliverable promises + the generated rule', () => {
    ...
    // vp.consumer.reactive: GENERATED by the consumer-policy compiler (D-19) -
    // valid only in server-stamped documents, never authorable.
    expect(diagnosticOnlyTypes().sort())
      .toEqual(['vp.consumer.reactive', 'vp.logic.gate', 'vp.notify.push']);
  });
