...
import { betrag, monatsZuordnung, periodenText, wertFehler, zeitText } from '../werteEingabe';
...
export function AblesungDialog({ kennzeichen, einheit, zone, alle, alt, onClose, onSaved }: { kennzeichen: string; einheit: string; zone: string; alle: Ablesung[]; alt: Ablesung | null; onClose: () => void; onSaved: (a: AblesungAntwort) => void }) {
  ...
  const speichern = async () => {
    ...
    if (!z.wert) f.tag = z.fehler ?? 'Bitte geben Sie einen Zeitpunkt an.';
  ...
  return <Modal open title={alt ? 'Ablesung berichtigen' : 'Ablesung eintragen'} onClose={() => { if (!busy) onClose(); }} footer={<><Button variant="outline" onClick={onClose} disabled={busy}>Abbrechen</Button>{erlaubt && <Button type="submit" form="ablesung-form" disabled={busy}>{busy ? 'Wird gespeichert …' : 'Speichern'}</Button>}</>}>
    <form id="ablesung-form" ref={form} className="vp-bz-form" onSubmit={e => { e.preventDefault(); void speichern(); }} noValidate>
      ...
      {alt ? <><p>{zeitText(alt.zeitpunkt, zone)} · Zeitzone: {zone}</p><p>Bisher wirksam: {betrag(alt.stand)} {einheit} · Fassung {alt.fassung}. Bis zu einer erforderlichen Freigabe gilt dieser Stand weiter.</p></> : <VpZeitpunktPicker value={zeit} zone={zone} onChange={v => { setZeit(v); setWahl('vorgabe'); }} disabled={busy} error={fehler.tag} />}
      <Input id="wert-stand" label={`Zählerstand (${einheit})`} inputMode="decimal" value={stand} autoFocus onChange={e => setStand(e.target.value)} disabled={busy} error={fehler.stand} />
      {zuordnung ? <>
      ...
      {alt && <Input id="wert-grund" label="Begründung" value={grund} onChange={e => setGrund(e.target.value)} disabled={busy} maxLength={500} error={fehler.grund} />}
