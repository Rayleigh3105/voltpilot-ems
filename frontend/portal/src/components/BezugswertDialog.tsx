...
export function BezugswertDialog({ bezug, werte, alt, standort, zone, onClose, onSaved }: { bezug: Bezugsgroesse; werte: BezugsgroesseWert[]; alt: BezugsgroesseWert | null; standort: string | null; zone: string; onClose: () => void; onSaved: (a: BezugswertAntwort) => void }) {
  ...
  return <Modal open title={alt ? 'Wert berichtigen' : 'Wert eingeben'} onClose={() => { if (!busy) onClose(); }} footer={<><Button variant="outline" disabled={busy} onClick={onClose}>Abbrechen</Button>{erlaubt && <Button type="submit" form="bezugswert-form" disabled={busy}>{busy ? 'Wird gespeichert …' : 'Speichern'}</Button>}</>}>
    <form id="bezugswert-form" ref={form} className="vp-bz-form" noValidate onSubmit={e => { e.preventDefault(); void speichern(); }}>
      ...
      <Input id="wert-wert" label={`Wert (${bezug.einheit})`} inputMode="decimal" autoFocus value={wert} onChange={e => setWert(e.target.value)} disabled={busy} error={fehler.wert} hint="Dezimalkomma und Tausenderpunkte sind erlaubt." />
      {alt && <Input id="wert-grund" label="Begründung" value={grund} onChange={e => setGrund(e.target.value)} disabled={busy} error={fehler.grund} maxLength={500} />}
