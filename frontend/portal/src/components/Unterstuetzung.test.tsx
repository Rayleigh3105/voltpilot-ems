...
vi.mock('../unterstuetzung', async original => ({ ...await original<typeof import('../unterstuetzung')>(), unterstuetzungApi: {
  standorte: vi.fn(), liste: vi.fn(), anfragen: vi.fn(), hinweise: vi.fn(), gewaehren: vi.fn(), verlaengern: vi.fn(), beenden: vi.fn(), ablehnen: vi.fn(), gelesen: vi.fn(), aktualisieren: vi.fn().mockResolvedValue(undefined),
} }));
beforeEach(() => {
  ...
  vi.mocked(unterstuetzungApi.standorte).mockResolvedValue(rechteSeed().me.standorte);
...
it('N4: Standort ist Pflicht, 12 Kalendermonate einschließlich Schaltjahrgrenze', async () => {
  ...
  await waitFor(() => expect(screen.getByRole('button', { name: 'Unterstützung gewähren', exact: true })).toBeEnabled());
...
it('Anfrage: Bestätigung übergibt die gewählte Anfrage und zeigt kein Partnerpasswort', async () => {
  ...
  await waitFor(() => expect(screen.getByRole('button', { name: 'Unterstützung bestätigen' })).toBeEnabled());
