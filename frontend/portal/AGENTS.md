...
- Geräteseitiger Summenwert-Assistent (`components/SummenwertAssistent.tsx`, Karte `components/GeraetPvProduktion.tsx`): baut aus dem VOLLEN Register-Inventar des Geräts (`api.measurementCatalog` + `measurementSelection`, NICHT nur `messkanaele`) einen „Gesamtwert" und ordnet ihn über die Rollen-API (`geraetRolle`/`rolleZuordnen`, PR 758) der Rolle PV-Produktion zu („verwenden als"). Reine Hälften: `summenwertQuellen.ts` (Gruppen/Guard/Suche) + `registerAbbildung.ts`. Nicht summierbare Register bleiben sichtbar, aber gesperrt mit Grund; das Registerwort ist „Register"/„beobachten".
